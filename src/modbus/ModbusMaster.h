...
{
    uint8_t Func;
    uint16_t Addr;   ///< slave register from which to read
    uint16_t Qty;    ///< quantity of words to read
    uint16_t Value;  // en caso de write register
} ModbusSlaveParams_t;
...
class ModbusMaster
{
   public:
    ...
    void SetSlaveID(uint8_t slave);  // agregado por mi (javier)
    void SetTimeout(uint16_t ms);    // agregado por mi (javier)

    static const uint8_t ASCII_FORMAT = 0;
    static const uint8_t RTU_FORMAT   = 1;
    void SetFormat(uint8_t format);  // agregado por mi (javier)
    String GetErrorStr(int e);       // agregado por mi (javier)
    ...
    */
    ...
    // agrego otra para manejar slaves.
    uint8_t ModbusSlaveRead(ModbusSlaveParams_t& params, uint32_t timeoutMs);
    uint8_t ModBusSlaveWriteHoldingRegisters(uint16_t u16WriteAddress, uint16_t* DataBuffer, uint8_t dataSize);
   ...
   private:
    ...
    uint16_t ku16MBResponseTimeout;  ///< Modbus timeout [milliseconds] (javier) ahora se setea de afuera.
