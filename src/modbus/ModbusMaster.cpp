...
ModbusMaster::ModbusMaster()
{
    ...
    _preTransmission  = 0;
    _postTransmission = 0;
    _format           = RTU_FORMAT;  // por default, pero se puede cambiar (javier)
}

// agregado por mi (javier)
void ModbusMaster::SetSlaveID(uint8_t slave) { _u8MBSlave = slave; }
void ModbusMaster::SetTimeout(uint16_t ms) { ku16MBResponseTimeout = ms; }
...
void ModbusMaster::begin(uint8_t slave, Stream &serial)
{
    ...
    ku16MBResponseTimeout   = 1000;  // por default, pero se puede cambiar (javier)
...
uint8_t ModbusMaster::available() { return _u8ResponseBufferLength - _u8ResponseBufferIndex; }
...
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t u8MBFunction)
{
    ...
    if (_format == ASCII_FORMAT)
    {
        // append LRC (checksum)
        // Sume todos los bytes de datos del mensaje (antes de convertir a ASCII y sin los dos puntos iniciales y CR/LF final).
        int LRC = 0;
        for (int i = 0; i < u8ModbusADUSize; i++)
        {
            LRC -= u8ModbusADU[i];
        }
        u8ModbusADU[u8ModbusADUSize++] = (uint8_t)(LRC & 0xFF);

        // dividir cada byte en 2 bytes ascii hex:
        uint8_t tempADU[512];
        int asize = u8ModbusADUSize;
        memcpy(tempADU, u8ModbusADU, asize);

        u8ModbusADUSize                = 0;
        u8ModbusADU[u8ModbusADUSize++] = ':';
        for (int i = 0; i < asize; i++)
        {
            u8ModbusADU[u8ModbusADUSize++] = (uint8_t)(tempADU[i] / 16);
            u8ModbusADU[u8ModbusADUSize++] = (uint8_t)(tempADU[i] % 16);
        }
        u8ModbusADU[u8ModbusADUSize++] = 0x0D;  // '\r'
        u8ModbusADU[u8ModbusADUSize++] = 0x0A;  // '\n'
    }
    else
    {
        // append CRC
    ...
    while (u8BytesLeft && !u8MBStatus)
    {
        if (_serial->available())
        {
            ...
            u8BytesLeft--;
#ifdef DEBUG_LOG
            Serial.printf("%02X ", u8ModbusADU[u8ModbusADUSize - 1]);
#endif
#if __MODBUSMASTER_DEBUG__
...
#ifdef DEBUG_LOG
    Serial.print("]");
    if (u8MBStatus) Serial.printf(" err %02X", u8MBStatus);
    Serial.println();
#endif
    _u8TransmitBufferIndex  = 0;
    u16TransmitBufferLength = 0;
...
/*
   http://www.tolaemon.com/docs/modbus.htm#func_3_4
    ...
    peticion del MASTER - 03 read register:
        ID   FUNC   ADDR    CANT    CRC
    P:[ 01 ][ 03 ][ 0100 ][ 0002 ][ C434 ]
        0     1     2-3     4-5     6-7

    respuesta del SLAVE:
        ID   FUNC  bytes  +..datos...+    CRC
    R:[ 01 ][ 03 ][ 04 ][ 2081 ][ 0000 ][ 76F1 ]
        0     1     2     3-4     5-6     7-8

    ----------------------
    peticion del MASTER - 06 write register:
        ID   FUNC   ADDR    VALUE   CRC
    P:[ 01 ][ 06 ][ 0100 ][ 0002 ][ C434 ]
        0     1     2-3     4-5     6-7

    respuesta del SLAVE:
        ID   FUNC  Address  Value    CRC
    R:[ 01 ][ 03 ][ 0C20 ][ 8100 ][ 0076 ]
        0     1     2-3     4-5     6-7
*/
uint8_t ModbusMaster::ModbusSlaveRead(ModbusSlaveParams_t &params, uint32_t timeoutMs)
{
    // habilito Rx:
    if (_postTransmission) _postTransmission();

    int temp;
    uint8_t TAMANIO_TRAMA    = 8;
    uint8_t u8ModbusADU[10]  = {0};  // leo la trama aca....
    uint16_t u8ModbusADUSize = 0;

    uint32_t T = millis() + timeoutMs;
    while (T > millis() && u8ModbusADUSize < TAMANIO_TRAMA)
    {
        temp = -1;
        if (_serial->available()) temp = _serial->read();
        if (temp != -1)
        {
            u8ModbusADU[u8ModbusADUSize] = (uint8_t)temp;
            ++u8ModbusADUSize;
        }
    }

    // lei todo o dio timeout.....

    if (u8ModbusADUSize != TAMANIO_TRAMA)
        return ku8MBResponseTimedOut;
    else if (u8ModbusADU[0] != _u8MBSlave)
        return ku8MBInvalidSlaveID;
    else
    {
        // calculate CRC
        uint16_t u16CRC = 0xFFFF;
        for (int i = 0; i < (TAMANIO_TRAMA - 2); i++) u16CRC = crc16_update(u16CRC, u8ModbusADU[i]);
        if ((lowByte(u16CRC) != u8ModbusADU[TAMANIO_TRAMA - 2] || highByte(u16CRC) != u8ModbusADU[TAMANIO_TRAMA - 1])) return ku8MBInvalidCRC;

        params.Func = u8ModbusADU[1];

        if (params.Func == ku8MBReadHoldingRegisters || params.Func == ku8MBReadInputRegisters)
        {
            params.Addr  = word(u8ModbusADU[2], u8ModbusADU[3]);
            params.Qty   = word(u8ModbusADU[4], u8ModbusADU[5]);
            params.Value = 0;
        }
        else if (params.Func == ku8MBWriteSingleRegister)
        {
            params.Addr  = word(u8ModbusADU[2], u8ModbusADU[3]);
            params.Value = word(u8ModbusADU[4], u8ModbusADU[5]);
            params.Qty   = 0;
        }
    }
...
uint8_t ModbusMaster::ModBusSlaveWriteHoldingRegisters(uint16_t u16WriteAddress, uint16_t *DataBuffer, uint8_t dataSize)
{
    uint8_t ADU[256];
    uint8_t idx = 0;
    uint8_t i;
    ...
    // assemble Modbus Request Application Data Unit
    ADU[idx++] = _u8MBSlave;    // ID
    ADU[idx++] = 0x03;          // Func write reg.
    ADU[idx++] = dataSize * 2;  // es en bytes!
    for (size_t i = 0; i < dataSize; i++)
    {
        ADU[idx++] = highByte(DataBuffer[i]);
        ADU[idx++] = lowByte(DataBuffer[i]);
    }
    u16CRC = 0xFFFF;
    for (i = 0; i < idx; i++) u16CRC = crc16_update(u16CRC, ADU[i]);
    ADU[idx++] = lowByte(u16CRC);
    ADU[idx++] = highByte(u16CRC);

    if (_preTransmission) _preTransmission();
    for (i = 0; i < idx; i++) _serial->write(ADU[i]);
    _serial->flush();
    if (_postTransmission) _postTransmission();

#ifdef DEBUG_LOG
    Serial.print("[");
    for (i = 0; i < idx; i++) Serial.printf("%02X", ADU[i]);
    Serial.println("]");
#endif
...
void ModbusMaster::SetFormat(uint8_t format) { _format = format; }

String ModbusMaster::GetErrorStr(int e)
{
    switch (e)
    {
        case 0xe0:
